...
class spatial_adaptive_operator(nn.Module):
    def __init__(self, d_model, n_points=4, points_type=0, sparse_topk=0):
        super().__init__()
    ...
    def forward(self, x, deltaA, H=None, W=None, points=None):
        ...
        if self.sparse_topk is not None and self.sparse_topk > 0 and self.sparse_topk < feature_aggre_other.shape[-1]:
            ...
            score_for_topk = feature_aggre_other.abs().mean(dim=1, keepdim=True)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.pattern == 'LoDSA':
            ...
            self.sa_op = spatial_adaptive_operator(d_model=self.d_inner, n_points=4, points_type=spatial_flag, sparse_topk=2)
        self.kwargs = kwargs
