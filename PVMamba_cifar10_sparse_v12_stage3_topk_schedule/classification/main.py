...
def main(config, args):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        # ===== sparse top-k schedule =====
        if hasattr(model, 'module'):
            if hasattr(model.module, 'update_sparse_schedule'):
                model.module.update_sparse_schedule(epoch)
        else:
            if hasattr(model, 'update_sparse_schedule'):
                model.update_sparse_schedule(epoch)
