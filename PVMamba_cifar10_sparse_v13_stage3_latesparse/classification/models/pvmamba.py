...
class spatial_adaptive_operator(nn.Module):
    def __init__(self, d_model, n_points=4, points_type=0, sparse_topk=2, enable_sparse=False):
        super().__init__()
    ...
    def set_sparse_topk(self, sparse_topk):
        self.sparse_topk = sparse_topk
    ...
    def forward(self, x, deltaA, H=None, W=None, points=None):
        ...
        feature_weight_tree_sampling = spatial_adaptive_sampling_core_pytorch(
            feature_weight_in.unsqueeze(2).contiguous(), spatial_shape, sampling_locations
        )  # [B, C + ?, N, P]

        feature_tree_sampling = feature_weight_tree_sampling[:, :C, ...]
        weight_tree_sampling = feature_weight_tree_sampling[:, C:, ...]

        base_weight = deltaA.transpose(1, 2).unsqueeze(-1)   # [B, ?, N, 1]
        score = weight_tree_sampling - base_weight           # [B, ?, N, P]

        feature_aggre_other = feature_tree_sampling * score  # [B, C, N, P]

        if self.enable_sparse and self.sparse_topk is not None and self.sparse_topk > 0 and self.sparse_topk < feature_aggre_other.shape[-1]:
            k = min(self.sparse_topk, feature_aggre_other.shape[-1])

            # 用 score 的绝对值做 top-k 选择
            score_abs = score.abs()

            # 如果 weight 通道数不止 1，先对权重通道做均值，得到 [B, 1, N, P]
            if score_abs.shape[1] > 1:
                score_for_topk = score_abs.mean(dim=1, keepdim=True)
            else:
                score_for_topk = score_abs

            topk_val, topk_idx = torch.topk(score_for_topk, k=k, dim=-1)

            mask = torch.zeros_like(score_for_topk)
            mask.scatter_(-1, topk_idx, 1.0)

            # 广播到 feature_aggre_other
            feature_aggre_other = feature_aggre_other * mask

            denom = mask.sum(dim=-1, keepdim=False).clamp(min=1.0)   # [B, 1, N]
            feature_in = (feature_aggre_other.sum(dim=-1) / denom).transpose(1, 2)
        else:
            feature_in = feature_aggre_other.mean(-1).transpose(1, 2)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.pattern == 'LoDSA':
            ...
            self.sa_op = spatial_adaptive_operator(d_model=self.d_inner, n_points=4, points_type=spatial_flag, sparse_topk=sparse_topk, enable_sparse=enable_sparse)
        self.kwargs = kwargs

    def set_sparse_topk(self, sparse_topk):
        self.sparse_topk = sparse_topk
        if hasattr(self, 'sa_op'):
            self.sa_op.set_sparse_topk(sparse_topk)

    def set_enable_sparse(self, enable_sparse):
        self.enable_sparse = enable_sparse
        if hasattr(self, 'sa_op'):
            self.sa_op.enable_sparse = enable_sparse
...
class PVMAMBABlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., drop=0., drop_path=0.,
                 ...
                 enable_sparse=False, sparse_topk=2,
                 **kwargs):
        super().__init__()
        self.dim = dim
        ...
        if kwargs.get('token_mixer_types', 'LoDSA') == 'attention':
            ...
            self.token_mixer = mamba_op(d_model=dim, expand=ssd_expansion, headdim= dim*ssd_expansion // num_heads,
                                ...
                                blk_idx=blk_idx, spatial_flag=spatial_flag, enable_sparse=enable_sparse, sparse_topk=sparse_topk,  **kwargs)
    ...
    def set_sparse_topk(self, sparse_topk):
        if hasattr(self.token_mixer, 'set_sparse_topk'):
            self.token_mixer.set_sparse_topk(sparse_topk)

    def set_enable_sparse(self, enable_sparse):
        if hasattr(self.token_mixer, 'set_enable_sparse'):
            self.token_mixer.set_enable_sparse(enable_sparse)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 spatial_flag=None, enable_sparse=False, sparse_topk=2, **kwargs):
        ...
        self.blocks = nn.ModuleList([
            PVMAMBABlock(dim=dim, input_resolution=input_resolution, num_heads=num_heads,
                      ...
                      blk_idx= i, spatial_flag= spatial_flag[i] if kwargs['token_mixer_types'] =='SaDSA' else None,
                      enable_sparse=enable_sparse, sparse_topk=sparse_topk, **kwargs)
    ...
    def set_sparse_topk(self, sparse_topk):
        for blk in self.blocks:
            if hasattr(blk, 'set_sparse_topk'):
                blk.set_sparse_topk(sparse_topk)

    def set_enable_sparse(self, enable_sparse):
        for blk in self.blocks:
            if hasattr(blk, 'set_enable_sparse'):
                blk.set_enable_sparse(enable_sparse)
...
class PVMAMBA(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            stage_enable_sparse = (i_layer == 2 and self.token_mixer_types[i_layer] == 'SaDSA')
            stage_sparse_topk = 2
            
            print(f"[Stage {i_layer + 1}] token_mixer={self.token_mixer_types[i_layer]}, enable_sparse={stage_enable_sparse}, sparse_topk={stage_sparse_topk}")
            ...
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               enable_sparse=stage_enable_sparse,
                               sparse_topk=stage_sparse_topk,
                               **kwargs)
    ...
    def set_stage3_sparse_topk(self, sparse_topk):
        if len(self.layers) > 2:
            self.layers[2].set_sparse_topk(sparse_topk)

    def set_stage3_sparse_enable(self, enable_sparse):
        if len(self.layers) > 2:
            self.layers[2].set_enable_sparse(enable_sparse)

    def update_sparse_schedule(self, epoch):
        # v13: 前10个epoch关闭sparse，后20个epoch开启stage3 sparse topk=2
        if epoch < 10:
            enable_sparse = False
            new_topk = 2
        else:
        ...
        self.set_stage3_sparse_enable(enable_sparse)
        self.set_stage3_sparse_topk(new_topk)

        print(f"[SparseSchedule] epoch={epoch}, stage3_enable_sparse={enable_sparse}, stage3_topk={new_topk}")
...
class Backbone_PVMAMBA(PVMAMBA):
    ...
    def forward(self, x):
        ...
        def layer_forward(l, x, H=None, W=None, points=None):
            for blk in l.blocks:
                x, points = blk(x, H, W, points)
        ...
        for i, layer in enumerate(self.layers):
            o, x, points = layer_forward(layer, x, H, W, points)  # (B, H, W, C)
