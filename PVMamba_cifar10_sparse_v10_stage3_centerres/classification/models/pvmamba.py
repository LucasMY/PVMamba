...
class spatial_adaptive_operator(nn.Module):
    def __init__(self, d_model, n_points=4, points_type=0, sparse_topk=2, enable_sparse=False, sparse_alpha=0.5):
        super().__init__()
    ...
    def forward(self, x, deltaA, H=None, W=None, points=None):
        ...
        if self.enable_sparse and self.sparse_topk is not None and self.sparse_topk > 0 and self.sparse_topk < feature_aggre_other.shape[-1]:
            ...
            denom = mask.sum(dim=-1, keepdim=False).clamp(min=1.0)
            sparse_feature = (feature_aggre_other.sum(dim=-1) / denom).transpose(1, 2)
        else:
            sparse_feature = feature_aggre_other.mean(-1).transpose(1, 2)

        # out = feature_in.contiguous()
        # return out, sampling_locations

        if self.enable_sparse:
            feature_in = center_feature + self.sparse_alpha * sparse_feature
        else:
            feature_in = sparse_feature

        out = feature_in.contiguous()
        return out, sampling_locations

        if self.enable_sparse and self.training and torch.rand(1).item() < 1e-4:
            print(f"[CenterResSparse] alpha={self.sparse_alpha}, topk={self.sparse_topk}")
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.pattern == 'LoDSA':
            ...
            self.sa_op = spatial_adaptive_operator(d_model=self.d_inner, n_points=4, points_type=spatial_flag, sparse_topk=sparse_topk, enable_sparse=enable_sparse, sparse_alpha=sparse_alpha)
        self.kwargs = kwargs
...
class PVMAMBABlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., drop=0., drop_path=0.,
                 ...
                 enable_sparse=False, sparse_topk=2, sparse_alpha=0.5,
                 **kwargs):
        super().__init__()
        self.dim = dim
        ...
        if kwargs.get('token_mixer_types', 'LoDSA') == 'attention':
            ...
            self.token_mixer = mamba_op(d_model=dim, expand=ssd_expansion, headdim= dim*ssd_expansion // num_heads,
                                ...
                                enable_sparse=enable_sparse, sparse_topk=sparse_topk, sparse_alpha=sparse_alpha,  **kwargs)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 spatial_flag=None, enable_sparse=False, sparse_topk=2, sparse_alpha=0.5,  **kwargs):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               sparse_alpha=stage_sparse_alpha,
                               **kwargs)
