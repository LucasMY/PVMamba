...
def build_model(config, is_pretrain=False):
    ...
    if model_type in ["pvmamba"]:
        model = PVMAMBA(
            ...
            sparse_layers=config.MODEL.PVMAMBA.SPARSE_LAYERS,
            sparse_hidden_ratio=config.MODEL.PVMAMBA.SPARSE_HIDDEN_RATIO,
            sparse_threshold=config.MODEL.PVMAMBA.SPARSE_THRESHOLD,
            hard_sparse=config.MODEL.PVMAMBA.HARD_SPARSE,
            sparse_alpha=config.MODEL.PVMAMBA.SPARSE_ALPHA,
            sparse_gamma=config.MODEL.PVMAMBA.SPARSE_GAMMA,
