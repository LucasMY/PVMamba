...
#软稀疏门控
# class SparseStateGate(nn.Module):
...
class SparseChannelGate(nn.Module):
    def __init__(self, dim, hidden_ratio=0.25):
        super().__init__()
        hidden_dim = max(8, int(dim * hidden_ratio))
        self.norm = nn.LayerNorm(dim)
    ...
    def forward(self, x):
        # x: [B, L, C]
        g = x.mean(dim=1, keepdim=True)   # [B, 1, C]
        g = self.norm(g)
        g = self.fc1(g)
        g = self.act(g)
        g = self.fc2(g)
...
class PVMAMBABlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., drop=0., drop_path=0.,
                 ...
                 sparse_enable=False, sparse_hidden_ratio=0.25, sparse_threshold=0.0,
                 hard_sparse=False, layer_idx=0, sparse_alpha=0.05, sparse_gamma=1.0, **kwargs):
        super().__init__()
        self.dim = dim
        ...
        self.layer_idx = layer_idx
        self._last_sparse_gate = None

        self.sparse_alpha = sparse_alpha
        self.sparse_gamma = sparse_gamma

        if self.sparse_enable:
            self.sparse_gate = SparseChannelGate(dim, hidden_ratio=sparse_hidden_ratio)
        else:
            self.sparse_gate = None

        # if self.sparse_enable:
        #     self.sparse_gate = SparseChannelGate(dim, hidden_ratio=sparse_hidden_ratio)
        #     self.sparse_alpha = nn.Parameter(torch.tensor(0.05))
        # else:
        #     self.sparse_gate = None
        #     self.sparse_alpha = None
    ...
    def forward(self, x, H=None, W=None, points=None):
        ...
        mix_out, points = self.token_mixer(x, H, W, points)

        # if self.sparse_gate is not None:
        #     gate = self.sparse_gate(x)   # 用 norm1 之后的 x 打分更稳

        #     if self.hard_sparse and self.sparse_threshold > 0:
        #         gate = gate * (gate > self.sparse_threshold).float()
        #     elif self.sparse_threshold > 0:
        #         gate = gate * (gate > self.sparse_threshold).float()

        #     # mix_out = mix_out * gate
        #     mix_out = mix_out * (1.0 + 0.1 * (gate - 0.5))
        #     self._last_sparse_gate = gate
        # else:
        #     self._last_sparse_gate = None

        # if self.sparse_gate is not None:
        #     gate = self.sparse_gate(x)   # [B, 1, C]
        ...
        #     alpha = torch.clamp(self.sparse_alpha, 0.0, 0.2)
        #     mix_out = mix_out * (1.0 + alpha * (gate - 0.5))
        #     self._last_sparse_gate = gate
        # else:
        #     self._last_sparse_gate = None
        ...
        if self.sparse_gate is not None:
            ...
            gate = gate.pow(self.sparse_gamma)
            ...
            mix_out = mix_out * (1.0 + self.sparse_alpha * (gate - 0.5))
            self._last_sparse_gate = gate
        else:
            self._last_sparse_gate = None

        x = shortcut + self.drop_path(mix_out)
        x = x + self.cpe2(x.reshape(B, H, W, C).permute(0, 3, 1, 2)).flatten(2).permute(0, 2, 1)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 spatial_flag=None, layer_idx=0,
                 sparse_enable=False, sparse_hidden_ratio=0.25,
                 sparse_threshold=0.0, hard_sparse=False,
                 sparse_alpha=0.05, sparse_gamma=1.0, **kwargs):
        ...
        self.blocks = nn.ModuleList([
            PVMAMBABlock(
                ...
                sparse_hidden_ratio=sparse_hidden_ratio,
                sparse_threshold=sparse_threshold,
                hard_sparse=hard_sparse,
...
class PVMAMBA(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.sparse_hidden_ratio = sparse_hidden_ratio
        self.sparse_threshold = sparse_threshold
        self.hard_sparse = hard_sparse
        ...
        for i_layer in range(self.num_layers):
            kwargs['token_mixer_types'] = self.token_mixer_types[i_layer]
            use_sparse = i_layer in self.sparse_layers
            ...
            layer = BasicLayer(
                ...
                spatial_flag=spatial_flag[i_layer],
                layer_idx=i_layer,
                sparse_enable=use_sparse,
                sparse_hidden_ratio=self.sparse_hidden_ratio,
                sparse_threshold=self.sparse_threshold,
                hard_sparse=self.hard_sparse,
                sparse_alpha=self.sparse_alpha,
                sparse_gamma=self.sparse_gamma,
                **kwargs
            )
            self.layers.append(layer)
