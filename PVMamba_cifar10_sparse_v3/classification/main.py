...
def main(config, args):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if acc1 > max_accuracy:
            max_accuracy = acc1
            best_epoch = epoch
            logger.info(f'New max accuracy: {max_accuracy:.2f}% at epoch {epoch + 1}')
        ...
        if model_ema is not None:
            ...
            if acc1_ema > max_accuracy_ema:
                ...
                logger.info(f'New max accuracy ema: {max_accuracy_ema:.2f}% at epoch {epoch + 1}')
    ...
    logger.info(
        f"Final summary | last acc: {last_acc1:.2f}% | "
        f"best acc: {max_accuracy:.2f}% | best epoch: {best_epoch + 1}"
    )

    if model_ema is not None:
        logger.info(
            f"Final EMA summary | last acc: {last_acc1_ema:.2f}% | "
            f"best acc: {max_accuracy_ema:.2f}% | best epoch: {best_epoch_ema + 1}"
        )
...
def compute_sparse_loss(model):
    loss_sparse = 0.0
    count = 0

    for m in model.modules():
        if hasattr(m, "_last_sparse_gate") and m._last_sparse_gate is not None:
            loss_sparse = loss_sparse + m._last_sparse_gate.mean()
            count += 1

    if count == 0:
        # 返回一个和主 loss 同设备的 0，避免类型/设备问题
        try:
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn,
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            ...
            loss_sparse = compute_sparse_loss(model_for_loss)

            # 4) 总损失
            loss = loss_cls + ema_loss + config.TRAIN.LAMBDA_SPARSE * loss_sparse
        ...
        loss_meter.update(loss.item(), targets.size(0))
        cls_meter.update(loss_cls.item(), targets.size(0))
        if isinstance(loss_sparse, torch.Tensor):
            sparse_meter.update(loss_sparse.item(), targets.size(0))
        else:
            sparse_meter.update(float(loss_sparse), targets.size(0))
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f'loss_cls {cls_meter.val:.4f} ({cls_meter.avg:.4f})\t'
                f'loss_sparse {sparse_meter.val:.6f} ({sparse_meter.avg:.6f})\t'
