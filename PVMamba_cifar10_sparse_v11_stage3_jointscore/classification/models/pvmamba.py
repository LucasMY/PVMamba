...
class spatial_adaptive_operator(nn.Module):
    def __init__(self, d_model, n_points=4, points_type=0, sparse_topk=2, enable_sparse=False, joint_lambda=0.7):
        super().__init__()
    ...
    def forward(self, x, deltaA, H=None, W=None, points=None):
        ...
        if self.enable_sparse and self.sparse_topk is not None and self.sparse_topk > 0 and self.sparse_topk < feature_aggre_other.shape[-1]:
            ...
            score_abs = score.abs()
            if score_abs.shape[1] > 1:
                score_term = score_abs.mean(dim=1, keepdim=True)   # [B, 1, N, P]
            else:
                score_term = score_abs

            # 分支2：内容强度
            feature_term = feature_tree_sampling.abs().mean(dim=1, keepdim=True)  # [B, 1, N, P]

            # 联合打分
            score_for_topk = self.joint_lambda * score_term + (1.0 - self.joint_lambda) * feature_term

            if self.training and torch.rand(1).item() < 1e-4:
                print(f"[JointScoreSparse] lambda={self.joint_lambda}, topk={k}")
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.pattern == 'LoDSA':
            ...
            self.sa_op = spatial_adaptive_operator(d_model=self.d_inner, n_points=4, points_type=spatial_flag, sparse_topk=sparse_topk, enable_sparse=enable_sparse, joint_lambda=joint_lambda)
        self.kwargs = kwargs
...
class PVMAMBABlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., drop=0., drop_path=0.,
                 ...
                 enable_sparse=False, sparse_topk=2, joint_lambda=0.7,
                 **kwargs):
        super().__init__()
        self.dim = dim
        ...
        if kwargs.get('token_mixer_types', 'LoDSA') == 'attention':
            ...
            self.token_mixer = mamba_op(d_model=dim, expand=ssd_expansion, headdim= dim*ssd_expansion // num_heads,
                                ...
                                enable_sparse=enable_sparse, sparse_topk=sparse_topk, joint_lambda=joint_lambda,  **kwargs)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 spatial_flag=None, enable_sparse=False, sparse_topk=2, joint_lambda=0.7,  **kwargs):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               joint_lambda=stage_joint_lambda,
                               **kwargs)
