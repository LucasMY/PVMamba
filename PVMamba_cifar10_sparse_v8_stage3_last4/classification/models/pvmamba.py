...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 spatial_flag=None, enable_sparse=False, sparse_topk=2, sparse_start_block=0, **kwargs):
        ...
        # build blocks
        if kwargs['token_mixer_types'] == 'SaDSA':
            print(f"[BasicLayer] depth={depth}, enable_sparse={enable_sparse}, sparse_topk={sparse_topk}, sparse_start_block={sparse_start_block}")

        self.blocks = nn.ModuleList()
        for i in range(depth):
            block_enable_sparse = (
                enable_sparse
                and kwargs['token_mixer_types'] == 'SaDSA'
                and i >= sparse_start_block
            )

            if kwargs['token_mixer_types'] == 'SaDSA':
                print(f"  [Block {i}] enable_sparse={block_enable_sparse}")
            ...
            blk = PVMAMBABlock(
                ...
                d_state=d_state,
                blk_idx=i,
                spatial_flag=spatial_flag[i] if kwargs['token_mixer_types'] == 'SaDSA' else None,
                enable_sparse=block_enable_sparse,
...
class PVMAMBA(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            # 只有 stage3，从第4号 block 开始启用 sparse（即后4个 block）
            if i_layer == 2:
                stage_sparse_start_block = 4
            else:
                stage_sparse_start_block = 999
            ...
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               sparse_start_block=stage_sparse_start_block,
                               **kwargs)
