...
class spatial_adaptive_operator(nn.Module):
    def __init__(self, d_model, n_points=4, points_type=0, sparse_topk=2, enable_sparse=False, sparse_tau=1.0):
        super().__init__()
        self.n_points = n_points

        self.enable_sparse = enable_sparse
        self.sparse_tau = sparse_tau
    ...
    def forward(self, x, deltaA, H=None, W=None, points=None):
        ...
        if self.enable_sparse and self.sparse_topk is not None and self.sparse_topk > 0 and self.sparse_topk < feature_aggre_other.shape[-1]:
            ...
            # 1) 用 |score| 做 top-k 选择
            score_abs = score.abs()

            # 若 weight 通道数 > 1，先压成单通道打分
            if score_abs.shape[1] > 1:
                score_for_topk = score_abs.mean(dim=1, keepdim=True)   # [B, 1, N, P]
            else:
                score_for_topk = score_abs                              # [B, 1, N, P] or [B, ?, N, P]

            topk_val, topk_idx = torch.topk(score_for_topk, k=k, dim=-1)   # [B, 1, N, k]

            # 2) 取出 top-k 对应的 feature_aggre_other
            topk_feature = torch.gather(
                feature_aggre_other,
                dim=-1,
                index=topk_idx.expand(-1, feature_aggre_other.shape[1], -1, -1)
            )   # [B, C, N, k]

            # 3) 对 top-k 分数做 softmax 权重
            # topk_weight = torch.softmax(topk_val, dim=-1)   # [B, 1, N, k]
            topk_weight = torch.softmax(topk_val / self.sparse_tau, dim=-1)

            # 4) 加权聚合
            feature_in = (topk_feature * topk_weight).sum(dim=-1).transpose(1, 2)
        else:
            feature_in = feature_aggre_other.mean(-1).transpose(1, 2)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.pattern == 'LoDSA':
            ...
            self.sa_op = spatial_adaptive_operator(d_model=self.d_inner, n_points=4, points_type=spatial_flag, sparse_topk=sparse_topk, enable_sparse=enable_sparse, sparse_tau=sparse_tau)
        self.kwargs = kwargs
...
class PVMAMBABlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., drop=0., drop_path=0.,
                 ...
                 enable_sparse=False, sparse_topk=2, sparse_tau=1.0,
                 **kwargs):
        super().__init__()
        self.dim = dim
        ...
        if kwargs.get('token_mixer_types', 'LoDSA') == 'attention':
            ...
            self.token_mixer = mamba_op(d_model=dim, expand=ssd_expansion, headdim= dim*ssd_expansion // num_heads,
                                ...
                                blk_idx=blk_idx, spatial_flag=spatial_flag, enable_sparse=enable_sparse, sparse_topk=sparse_topk, sparse_tau=sparse_tau,  **kwargs)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 spatial_flag=None, enable_sparse=False, sparse_topk=2, sparse_tau=1.0,  **kwargs):
        ...
        self.blocks = nn.ModuleList([
            PVMAMBABlock(dim=dim, input_resolution=input_resolution, num_heads=num_heads,
                      ...
                      enable_sparse=enable_sparse, sparse_topk=sparse_topk, sparse_tau=sparse_tau,  **kwargs)
