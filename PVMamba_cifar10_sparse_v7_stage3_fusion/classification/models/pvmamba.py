...
class spatial_adaptive_operator(nn.Module):
    def __init__(self, d_model, n_points=4, points_type=0, sparse_topk=2, enable_sparse=False, sparse_beta=0.5):
        super().__init__()
    ...
    def forward(self, x, deltaA, H=None, W=None, points=None):
        ...
        # 1) 原始 dense 聚合始终保留
        dense_out = feature_aggre_other.mean(-1).transpose(1, 2)   # [B, N, C]

        # 2) 只有启用 sparse 时，才额外构建 sparse 分支
        if self.enable_sparse and self.sparse_topk is not None and self.sparse_topk > 0 and self.sparse_topk < feature_aggre_other.shape[-1]:
            k = min(self.sparse_topk, feature_aggre_other.shape[-1])

            score_abs = score.abs()

            # 若权重通道不止1，先汇聚成每个 point 的分数
            if score_abs.shape[1] > 1:
                score_for_topk = score_abs.mean(dim=1, keepdim=True)   # [B, 1, N, P]
            else:
                score_for_topk = score_abs

            _, topk_idx = torch.topk(score_for_topk, k=k, dim=-1)

            mask = torch.zeros_like(score_for_topk)
            mask.scatter_(-1, topk_idx, 1.0)

            masked_feature = feature_aggre_other * mask
            denom = mask.sum(dim=-1, keepdim=False).clamp(min=1.0)     # [B, 1, N]
            sparse_out = (masked_feature.sum(dim=-1) / denom).transpose(1, 2)   # [B, N, C]

            # 3) dense 主干 + sparse 增强支路 (dense+β⋅(sparse−dense))
            feature_in = dense_out + self.sparse_beta * (sparse_out - dense_out)
        else:
            feature_in = dense_out
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.pattern == 'LoDSA':
            ...
            self.sa_op = spatial_adaptive_operator(d_model=self.d_inner, n_points=4, points_type=spatial_flag, sparse_topk=sparse_topk, enable_sparse=enable_sparse, sparse_beta=sparse_beta)
        self.kwargs = kwargs
...
class PVMAMBABlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., drop=0., drop_path=0.,
                 ...
                 enable_sparse=False, sparse_topk=2, sparse_beta=0.5,
                 **kwargs):
        super().__init__()
        self.dim = dim
        ...
        if kwargs.get('token_mixer_types', 'LoDSA') == 'attention':
            ...
            self.token_mixer = mamba_op(d_model=dim, expand=ssd_expansion, headdim= dim*ssd_expansion // num_heads,
                                ...
                                blk_idx=blk_idx, spatial_flag=spatial_flag,
                                enable_sparse=enable_sparse, sparse_topk=sparse_topk, sparse_beta=sparse_beta,  **kwargs)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 spatial_flag=None, enable_sparse=False, sparse_topk=2, sparse_beta=0.5, **kwargs):
...
class PVMAMBA(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            ...
            stage_sparse_topk = 2
            stage_sparse_beta = 0.5
            
            print(f"[Stage {i_layer + 1}] token_mixer={self.token_mixer_types[i_layer]}, enable_sparse={stage_enable_sparse}, sparse_topk={stage_sparse_topk}, sparse_beta={stage_sparse_beta}")
